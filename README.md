...
- **零静默吞错**：所有 `except` 块都记录错误源、类型、上下文到日志
- **错误计数器**：每个数据源独立计数，连续 10 次失败自动推送告警
- **结构化日志**：JSON 格式 + 文件轮转，可 `jq` 查询或接入日志平台

### 安全控制

- **Bot 命令鉴权**：通过 `TG_ADMIN_IDS` 环境变量配置管理员白名单
- **未授权命令拒绝**：非白名单用户的命令请求会被记录并静默忽略
- **空白名单兼容**：不配置 = 不限制（向后兼容）

### 依赖与环境

- **版本锁定**：`requirements.txt` 使用范围约束 (`>=x.y,<z.0`)，避免意外升级
- **Docker 支持**：`docker compose up -d` 一键部署，环境隔离
- **配置校验**：启动时自检网络/API/TG/磁盘，任何异常立即日志告警

### HTTP 一致性

- **统一 HttpClient**：所有出站请求（含飞书）走统一的 Session 复用 + 重试 + UA 轮换
- **AI 调用独立**：AI provider 使用原生 requests（无自动重试），避免限流放大
- **限流退避**：429 响应触发指数退避，不打满供应商配额
